...
## 1.6、升级流程
![[Pasted image 20250622231517.png]]
如上所示，升级的流程从上往下执行。
1. IAP上位机发送起始帧后，下位机回复ACK，然后从App跳转到bootloader程序，在bootloader程序回复C给上位机， 表示正式接收升级固件的数据）。
2. IAP上位机收到ACK后，开始发送数据帧。每发一条数据帧都要等待下位机的回复（ACK或者其他功能帧）。
3. IAP上位机将最后一个数据帧发送完，收到下位机的ACK后。接着，发送功能帧EOT告诉下位机，文件传输结束。
4. 收到下位机的ACK后，发送最后的结束帧。接着，下位机收到结束帧后，继续回复最后一次的ACK。至此，IAP升级正式结束。
